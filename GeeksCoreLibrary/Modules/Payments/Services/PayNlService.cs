...
public class PayNlService : PaymentServiceProviderBaseService, IPaymentServiceProviderService, IScopedService
{
    ...
    public static string HandleSoftPos(PayNlSettingsModel payNlSettings, string invoiceNumber, decimal totalPrice, GclSettings gclSettings, bool offline = false, string returnUrl = "", string exchangeUrl = "", string transactionReference = "")
    {
        ...
        // Replace {totalPrice} in JSON if not offline
        if (!offline)
            transactionJson = transactionJson.Replace("\"{totalPrice}\"", ((int) Math.Round(totalPrice * 100)).ToString());

        // Encode each parameter
        var authenticationEncoded = offline ? "{invoiceNumber}" : HttpUtility.UrlEncode(invoiceNumber);
        var transactionEncoded = HttpUtility.UrlEncode(transactionJson);
        var layoutEncoded = HttpUtility.UrlEncode(layoutJson);
        
        // Set back variables after UrlEncoding
        if (offline) {
            transactionEncoded = transactionEncoded.Replace("%22%7btotalPrice%7d%22", "{totalPrice}"); // Also remove the double quotes around the total value, because this may not be a string
            transactionEncoded = transactionEncoded.Replace("%7binvoiceNumber%7d", "{invoiceNumber}");
        }
    ...
    public async Task<StatusUpdateResult> ProcessStatusUpdateAsync(OrderProcessSettingsModel orderProcessSettings, PaymentMethodSettingsModel paymentMethodSettings)
    {
        ...
        if (string.Equals(paymentMethodSettings.ExternalName, "softpos", StringComparison.OrdinalIgnoreCase))
        {
            try
            {
                ...
                {
                    await LogIncomingPaymentActionAsync(PaymentServiceProviders.PayNl, payNlOrderId, (int) restResponse.StatusCode, responseBody: restResponse.Content);
                    return new StatusUpdateResult
                    {
                        Successful = false,
                        Status = "error"
