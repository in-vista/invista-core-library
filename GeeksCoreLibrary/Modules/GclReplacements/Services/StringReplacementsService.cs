...
namespace GeeksCoreLibrary.Modules.GclReplacements.Services
{
    ...
    public class StringReplacementsService : IStringReplacementsService, IScopedService
    {
        ...
        public string DoHttpRequestReplacements(string input, bool forQuery = false, string defaultFormatter = "HtmlEncode")
        {
            ...
            {
                input = replacementsMediator.DoReplacements(input, QueryHelpers.ParseQuery(wiserUriOverride.Query), forQuery, defaultFormatter: defaultFormatter, isFromUnsafeSource: true);
            }
            else
            {
                input = replacementsMediator.DoReplacements(input, httpContextAccessor.HttpContext.Request.Query, forQuery, defaultFormatter: defaultFormatter, isFromUnsafeSource: true);
            }

            // POST variables.
            if (httpContextAccessor.HttpContext.Request.HasFormContentType)
            {
                input = replacementsMediator.DoReplacements(input, httpContextAccessor.HttpContext.Request.Form, forQuery, defaultFormatter: defaultFormatter, isFromUnsafeSource: true);
            }

            // Cookies.
            input = replacementsMediator.DoReplacements(input, httpContextAccessor.HttpContext.Request.Cookies, forQuery, defaultFormatter: defaultFormatter, isFromUnsafeSource: true);
